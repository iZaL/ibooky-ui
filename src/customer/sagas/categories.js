...
function* fetchCategories() {
  ...
  function* fetchPage(action) {
    try {
      let query = {
        query:`page=${action.params.page}`
      };
      const response = yield call(API.fetchPage,query);
      const normalized = normalize(response.data,Schema.pages);
      yield put({
        type: ACTION_TYPES.FETCH_PAGE_SUCCESS,
        entities: normalized.entities,
      });
    } catch (error) {
      yield put({type: ACTION_TYPES.FETCH_PAGE_FAILURE, error});
    }
  }
...
// Monitoring Sagas
  ...
  function* fetchPageMonitor() {
    yield takeLatest(
      ACTION_TYPES.FETCH_PAGE_REQUEST,
      fetchPage,
    );
  }
